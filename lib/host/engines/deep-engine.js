// ============================================================================
// deep-engine.js — 独立滑杆引擎：深度思考（Deep Thinking）
...
export class DeepEngine {
  constructor() {
    this.key = "deep";
    this.name = "深度思考";
    this.nameEn = "Deep Thinking";
    this.domain = "cognition";
    this.curve = "exp";
    this.scalarNote = "因果回滚深度：发现错误向上回溯的因果层数";
    this.toolPool = Object.freeze(['think-deep', 'context-inject', 'coherence-scan', 'dual-recheck', 'falsify-redteam', 'best-of', 'apex-tournament']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 2, fanout: 1, temp: 0.80, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['think-deep', 'context-inject'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 3, fanout: 1, temp: 0.70, budgetW: 2, checks: 2, scalar: 1, gain: 0.1545, tools: ['think-deep', 'context-inject'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 4, fanout: 2, temp: 0.60, budgetW: 3, checks: 3, scalar: 2, gain: 0.3078, tools: ['think-deep', 'context-inject', 'coherence-scan'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 6, fanout: 3, temp: 0.45, budgetW: 5, checks: 5, scalar: 2, gain: 0.5019, tools: ['think-deep', 'context-inject', 'coherence-scan', 'dual-recheck'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 9, fanout: 4, temp: 0.30, budgetW: 8, checks: 7, scalar: 3, gain: 0.7335, tools: ['think-deep', 'context-inject', 'coherence-scan', 'dual-recheck', 'falsify-redteam'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 13, fanout: 6, temp: 0.20, budgetW: 12, checks: 10, scalar: 5, gain: 1, tools: ['think-deep', 'context-inject', 'coherence-scan', 'dual-recheck', 'falsify-redteam', 'best-of'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return DeepEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = DeepEngine.TIER[t], prev = DeepEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = DeepEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = DeepEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return DeepEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new DeepEngine();
export default singleton;
export const ENGINE_KEY = "deep";
