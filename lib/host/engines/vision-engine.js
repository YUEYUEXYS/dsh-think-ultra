// ============================================================================
// vision-engine.js — 独立滑杆引擎：深度看图（Deep Vision）
...
export class VisionEngine {
  constructor() {
    this.key = "vision";
    this.name = "深度看图";
    this.nameEn = "Deep Vision";
    this.domain = "vision";
    this.curve = "exp";
    this.scalarNote = "视觉分区数：按 3×N 宫格逐区域精细描述";
    this.toolPool = Object.freeze(['vision-tile', 'image-crosscheck', 'ocr-transcribe', 'spatial-graph', 'subpixel-read']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 3, temp: 0.72, budgetW: 1, checks: 1, scalar: 3, gain: 0.0475, tools: ['vision-tile', 'image-crosscheck'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 6, temp: 0.64, budgetW: 2, checks: 2, scalar: 6, gain: 0.1545, tools: ['vision-tile', 'image-crosscheck'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 9, temp: 0.56, budgetW: 4, checks: 4, scalar: 9, gain: 0.3078, tools: ['vision-tile', 'image-crosscheck', 'ocr-transcribe'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 12, temp: 0.44, budgetW: 6, checks: 6, scalar: 12, gain: 0.5019, tools: ['vision-tile', 'image-crosscheck', 'ocr-transcribe', 'spatial-graph'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 5, fanout: 15, temp: 0.32, budgetW: 9, checks: 8, scalar: 15, gain: 0.7335, tools: ['vision-tile', 'image-crosscheck', 'ocr-transcribe', 'spatial-graph', 'subpixel-read'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 7, fanout: 21, temp: 0.22, budgetW: 14, checks: 12, scalar: 21, gain: 1, tools: ['vision-tile', 'image-crosscheck', 'ocr-transcribe', 'spatial-graph', 'subpixel-read'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VisionEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VisionEngine.TIER[t], prev = VisionEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VisionEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VisionEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VisionEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VisionEngine();
export default singleton;
export const ENGINE_KEY = "vision";
