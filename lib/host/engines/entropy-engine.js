// ============================================================================
// entropy-engine.js — 独立滑杆引擎：熵减监控（Entropy Control）
...
export class EntropyEngine {
  constructor() {
    this.key = "entropy";
    this.name = "熵减监控";
    this.nameEn = "Entropy Control";
    this.domain = "cognition";
    this.curve = "exp";
    this.scalarNote = "信息熵审计遍数（MDL 最小描述长度）";
    this.toolPool = Object.freeze(['entropy-prune', 'coherence-scan', 'mdl-entropy']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['entropy-prune', 'coherence-scan'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.62, budgetW: 1, checks: 2, scalar: 2, gain: 0.1545, tools: ['entropy-prune', 'coherence-scan'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 1, temp: 0.54, budgetW: 2, checks: 3, scalar: 3, gain: 0.3078, tools: ['entropy-prune', 'coherence-scan', 'mdl-entropy'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.42, budgetW: 3, checks: 4, scalar: 4, gain: 0.5019, tools: ['entropy-prune', 'coherence-scan', 'mdl-entropy'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 5, fanout: 2, temp: 0.32, budgetW: 5, checks: 6, scalar: 5, gain: 0.7335, tools: ['entropy-prune', 'coherence-scan', 'mdl-entropy'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 8, fanout: 3, temp: 0.24, budgetW: 7, checks: 8, scalar: 8, gain: 1, tools: ['entropy-prune', 'coherence-scan', 'mdl-entropy'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return EntropyEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = EntropyEngine.TIER[t], prev = EntropyEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = EntropyEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = EntropyEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return EntropyEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new EntropyEngine();
export default singleton;
export const ENGINE_KEY = "entropy";
