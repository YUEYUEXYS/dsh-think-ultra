// ============================================================================
// proof-engine.js — 独立滑杆引擎：形式化证明义务（Formal Proof Obligation）
// 领域：engineering；非线性族：指数增益 exp
// 工程主轴：档越深，走查轮数、路径覆盖与不变量证明越完整。
...
export class ProofEngine {
  constructor() {
    this.key = "proof";
    this.name = "形式化证明义务";
    this.nameEn = "Formal Proof Obligation";
    this.domain = "engineering";
    this.curve = "exp";
    this.scalarNote = "结论释放前必须闭合的证明义务数量";
    this.toolPool = Object.freeze(['formal-proof-close', 'obligation-discharge', 'proof-chain', 'formal-cohere', 'logic-proof', 'pro-math-derive']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 2, scalar: 1, gain: 0.0475, tools: ['formal-proof-close', 'obligation-discharge'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.62, budgetW: 2, checks: 3, scalar: 2, gain: 0.1545, tools: ['formal-proof-close', 'obligation-discharge', 'proof-chain'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.54, budgetW: 4, checks: 4, scalar: 4, gain: 0.3078, tools: ['formal-proof-close', 'obligation-discharge', 'proof-chain'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 5, fanout: 3, temp: 0.42, budgetW: 6, checks: 6, scalar: 6, gain: 0.5019, tools: ['formal-proof-close', 'obligation-discharge', 'proof-chain', 'formal-cohere'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 7, fanout: 4, temp: 0.30, budgetW: 10, checks: 9, scalar: 9, gain: 0.7335, tools: ['formal-proof-close', 'obligation-discharge', 'proof-chain', 'formal-cohere', 'logic-proof', 'pro-math-derive'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 11, fanout: 6, temp: 0.22, budgetW: 15, checks: 13, scalar: 13, gain: 1, tools: ['formal-proof-close', 'obligation-discharge', 'proof-chain', 'formal-cohere', 'logic-proof', 'pro-math-derive'] },
  ]);

  // 本引擎全部“交付前自检”候选池（按档位 checks 数取前 N 条）
  static CHECK_POOL = Object.freeze([
    "在类型、取值范围、业务语义三层都做输入校验",
    "异常路径、超时、资源释放都有明确交代",
    "排除越界、空值、除零、死循环与竞态条件",
    "关键行为可复算、可实跑、可回归验证",
    "依赖/调用图无环、无悬空节点、无隐藏回归面",
    "不使用 eval、硬编码密钥、字符串拼 SQL、路径拼接等反模式",
    "并发与缓存的失效时机、一致性边界写清楚",
    "每个公开行为给出前置条件与后置不变量",
    "改动的读/写/抛错路径集完整，副作用可枚举",
    "无法证明安全的路径显式标红并给隔离或回滚方案",
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ProofEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ProofEngine.TIER[t], prev = ProofEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ProofEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ProofEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ProofEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ProofEngine();
export default singleton;
export const ENGINE_KEY = "proof";
