// ============================================================================
// heal-engine.js — 独立滑杆引擎：失败自愈（Self-Heal）
...
export class HealEngine {
  constructor() {
    this.key = "heal";
    this.name = "失败自愈";
    this.nameEn = "Self-Heal";
    this.domain = "stability";
    this.curve = "lin";
    this.scalarNote = "恢复策略层级数（重试/回退/换路/拆分/熔断）";
    this.toolPool = Object.freeze(['root-cause-heal', 'failure-replay', 'exec-verify']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 1, gain: 0.1667, tools: ['root-cause-heal', 'failure-replay'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.64, budgetW: 1, checks: 2, scalar: 2, gain: 0.3333, tools: ['root-cause-heal', 'failure-replay'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.56, budgetW: 2, checks: 3, scalar: 3, gain: 0.5, tools: ['root-cause-heal', 'failure-replay', 'exec-verify'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.46, budgetW: 3, checks: 4, scalar: 4, gain: 0.6667, tools: ['root-cause-heal', 'failure-replay', 'exec-verify'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 5, fanout: 3, temp: 0.36, budgetW: 5, checks: 6, scalar: 5, gain: 0.8333, tools: ['root-cause-heal', 'failure-replay', 'exec-verify'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 4, temp: 0.26, budgetW: 7, checks: 8, scalar: 6, gain: 1, tools: ['root-cause-heal', 'failure-replay', 'exec-verify'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return HealEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = HealEngine.TIER[t], prev = HealEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = HealEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = HealEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return HealEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new HealEngine();
export default singleton;
export const ENGINE_KEY = "heal";
