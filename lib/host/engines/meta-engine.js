// ============================================================================
// meta-engine.js — 独立滑杆引擎：元认知（Metacognition）
...
export class MetaEngine {
  constructor() {
    this.key = "meta";
    this.name = "元认知";
    this.nameEn = "Metacognition";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "过程检查点数：每到检查点停下做二阶自省";
    this.toolPool = Object.freeze(['metacog-monitor', 'confidence-calib', 'bayes-calib', 'falsify-redteam']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 2, fanout: 1, temp: 0.72, budgetW: 1, checks: 2, scalar: 2, gain: 0.0475, tools: ['metacog-monitor', 'confidence-calib'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 3, fanout: 2, temp: 0.64, budgetW: 2, checks: 3, scalar: 3, gain: 0.1545, tools: ['metacog-monitor', 'confidence-calib', 'bayes-calib'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 4, fanout: 2, temp: 0.56, budgetW: 3, checks: 4, scalar: 4, gain: 0.3078, tools: ['metacog-monitor', 'confidence-calib', 'bayes-calib'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 6, fanout: 3, temp: 0.44, budgetW: 5, checks: 6, scalar: 6, gain: 0.5019, tools: ['metacog-monitor', 'confidence-calib', 'bayes-calib', 'falsify-redteam'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 8, fanout: 4, temp: 0.32, budgetW: 8, checks: 8, scalar: 8, gain: 0.7335, tools: ['metacog-monitor', 'confidence-calib', 'bayes-calib', 'falsify-redteam'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 12, fanout: 6, temp: 0.24, budgetW: 13, checks: 12, scalar: 12, gain: 1, tools: ['metacog-monitor', 'confidence-calib', 'bayes-calib', 'falsify-redteam'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return MetaEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = MetaEngine.TIER[t], prev = MetaEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = MetaEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = MetaEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return MetaEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new MetaEngine();
export default singleton;
export const ENGINE_KEY = "meta";
