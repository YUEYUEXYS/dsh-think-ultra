// ============================================================================
// sharpen-engine.js — 独立滑杆引擎：结论锐化收敛（Conclusion Sharpening）
// 领域：cognition；非线性族：指数增益 exp
...
export class SharpenEngine {
  constructor() {
    this.key = "sharpen";
    this.name = "结论锐化收敛";
    this.nameEn = "Conclusion Sharpening";
    this.domain = "cognition";
    this.curve = "exp";
    this.scalarNote = "消除模糊措辞与并列表述的锐化遍数";
    this.toolPool = Object.freeze(['sharpen-converge', 'decision-freeze', 'coherence-scan', 'judge-panel', 'dual-recheck', 'confidence-calib']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['sharpen-converge', 'decision-freeze'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.62, budgetW: 1, checks: 2, scalar: 2, gain: 0.1545, tools: ['sharpen-converge', 'decision-freeze'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 1, temp: 0.54, budgetW: 2, checks: 3, scalar: 3, gain: 0.3078, tools: ['sharpen-converge', 'decision-freeze', 'coherence-scan'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 2, temp: 0.42, budgetW: 3, checks: 4, scalar: 4, gain: 0.5019, tools: ['sharpen-converge', 'decision-freeze', 'coherence-scan'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 2, temp: 0.30, budgetW: 5, checks: 6, scalar: 6, gain: 0.7335, tools: ['sharpen-converge', 'decision-freeze', 'coherence-scan', 'judge-panel'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 3, temp: 0.22, budgetW: 8, checks: 9, scalar: 9, gain: 1, tools: ['sharpen-converge', 'decision-freeze', 'coherence-scan', 'judge-panel', 'dual-recheck', 'confidence-calib'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return SharpenEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = SharpenEngine.TIER[t], prev = SharpenEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = SharpenEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = SharpenEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return SharpenEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new SharpenEngine();
export default singleton;
export const ENGINE_KEY = "sharpen";
