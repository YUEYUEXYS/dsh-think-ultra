// ============================================================================
// agent-engine.js — 独立滑杆引擎：并行编排（Agent Topology）
// 领域：stability；非线性族：阶乘跃升 fact
...
export class AgentEngine {
  constructor() {
    this.key = "agent";
    this.name = "并行编排";
    this.nameEn = "Agent Topology";
    this.domain = "stability";
    this.curve = "fact";
    this.scalarNote = "可并行角色/分片数：规划/执行/审查分工";
    this.toolPool = Object.freeze(['agent-topology', 'branch-tree', 'best-of']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 1, gain: 0.0159, tools: ['agent-topology', 'branch-tree'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 2, temp: 0.66, budgetW: 1, checks: 2, scalar: 2, gain: 0.0476, tools: ['agent-topology', 'branch-tree'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 3, temp: 0.60, budgetW: 2, checks: 3, scalar: 3, gain: 0.1111, tools: ['agent-topology', 'branch-tree', 'best-of'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 4, temp: 0.50, budgetW: 3, checks: 4, scalar: 4, gain: 0.2381, tools: ['agent-topology', 'branch-tree', 'best-of'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 6, temp: 0.40, budgetW: 5, checks: 6, scalar: 6, gain: 0.4921, tools: ['agent-topology', 'branch-tree', 'best-of'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 8, fanout: 8, temp: 0.30, budgetW: 7, checks: 8, scalar: 8, gain: 1, tools: ['agent-topology', 'branch-tree', 'best-of'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return AgentEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = AgentEngine.TIER[t], prev = AgentEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = AgentEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = AgentEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return AgentEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new AgentEngine();
export default singleton;
export const ENGINE_KEY = "agent";
