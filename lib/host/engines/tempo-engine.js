// ============================================================================
// tempo-engine.js — 独立滑杆引擎：推理迭代密度（Reasoning Tempo）
...
export class TempoEngine {
  constructor() {
    this.key = "tempo";
    this.name = "推理迭代密度";
    this.nameEn = "Reasoning Tempo";
    this.domain = "cognition";
    this.curve = "exp";
    this.scalarNote = "单位时间内的时间盒推理冲刺数（用速度堆深度）";
    this.toolPool = Object.freeze(['rapid-iterate', 'branch-tree', 'think-deep', 'best-of', 'agent-topology', 'flash-branch', 'cognitive-tournament']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 2, fanout: 1, temp: 0.78, budgetW: 1, checks: 1, scalar: 2, gain: 0.0475, tools: ['rapid-iterate', 'branch-tree'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 3, fanout: 2, temp: 0.70, budgetW: 2, checks: 2, scalar: 3, gain: 0.1545, tools: ['rapid-iterate', 'branch-tree'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 5, fanout: 2, temp: 0.62, budgetW: 3, checks: 3, scalar: 5, gain: 0.3078, tools: ['rapid-iterate', 'branch-tree', 'think-deep'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 7, fanout: 3, temp: 0.50, budgetW: 5, checks: 5, scalar: 7, gain: 0.5019, tools: ['rapid-iterate', 'branch-tree', 'think-deep', 'best-of'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 10, fanout: 4, temp: 0.38, budgetW: 8, checks: 7, scalar: 10, gain: 0.7335, tools: ['rapid-iterate', 'branch-tree', 'think-deep', 'best-of', 'agent-topology'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 14, fanout: 6, temp: 0.28, budgetW: 12, checks: 10, scalar: 14, gain: 1, tools: ['rapid-iterate', 'branch-tree', 'think-deep', 'best-of', 'agent-topology', 'flash-branch'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return TempoEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = TempoEngine.TIER[t], prev = TempoEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = TempoEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = TempoEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return TempoEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new TempoEngine();
export default singleton;
export const ENGINE_KEY = "tempo";
