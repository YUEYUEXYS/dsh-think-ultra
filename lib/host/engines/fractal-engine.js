// ============================================================================
// fractal-engine.js — 独立滑杆引擎：分形嵌套深度（Fractal Nesting）
...
export class FractalEngine {
  constructor() {
    this.key = "fractal";
    this.name = "分形嵌套深度";
    this.nameEn = "Fractal Nesting";
    this.domain = "metacog";
    this.curve = "fact";
    this.scalarNote = "子节点复刻完整父拓扑的分形层数（每深一层复杂度翻一个数量级）";
    this.toolPool = Object.freeze(['fractal-nest', 'self-similar-recurse', 'pro-recursion-ctrl', 'cognitive-tournament', 'branch-tree', 'abstraction-lift']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.76, budgetW: 1, checks: 1, scalar: 1, gain: 0.0159, tools: ['fractal-nest', 'self-similar-recurse'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 2, temp: 0.70, budgetW: 2, checks: 2, scalar: 1, gain: 0.0476, tools: ['fractal-nest', 'self-similar-recurse'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 3, temp: 0.62, budgetW: 4, checks: 3, scalar: 2, gain: 0.1111, tools: ['fractal-nest', 'self-similar-recurse', 'pro-recursion-ctrl'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 5, temp: 0.50, budgetW: 7, checks: 5, scalar: 3, gain: 0.2381, tools: ['fractal-nest', 'self-similar-recurse', 'pro-recursion-ctrl', 'cognitive-tournament'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 8, temp: 0.38, budgetW: 12, checks: 7, scalar: 4, gain: 0.4921, tools: ['fractal-nest', 'self-similar-recurse', 'pro-recursion-ctrl', 'cognitive-tournament', 'branch-tree'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 13, temp: 0.28, budgetW: 19, checks: 11, scalar: 5, gain: 1, tools: ['fractal-nest', 'self-similar-recurse', 'pro-recursion-ctrl', 'cognitive-tournament', 'branch-tree', 'abstraction-lift'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return FractalEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = FractalEngine.TIER[t], prev = FractalEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = FractalEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = FractalEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return FractalEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new FractalEngine();
export default singleton;
export const ENGINE_KEY = "fractal";
