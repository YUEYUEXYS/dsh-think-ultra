// ============================================================================
// axiom-engine.js — 独立滑杆引擎：公理层覆写深度（Axiom Override Depth）
...
export class AxiomEngine {
  constructor() {
    this.key = "axiom";
    this.name = "公理层覆写深度";
    this.nameEn = "Axiom Override Depth";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "可被隔离覆写的底层公理/常识层数";
    this.toolPool = Object.freeze(['pro-axiom-transform', 'axiom-override', 'axiom-sandbox', 'first-principles-lift', 'proof-chain', 'logic-proof']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.76, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['pro-axiom-transform', 'axiom-override'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.70, budgetW: 2, checks: 2, scalar: 2, gain: 0.1545, tools: ['pro-axiom-transform', 'axiom-override'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.62, budgetW: 4, checks: 4, scalar: 3, gain: 0.3078, tools: ['pro-axiom-transform', 'axiom-override', 'axiom-sandbox'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 5, fanout: 3, temp: 0.50, budgetW: 6, checks: 6, scalar: 5, gain: 0.5019, tools: ['pro-axiom-transform', 'axiom-override', 'axiom-sandbox', 'first-principles-lift'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 7, fanout: 4, temp: 0.38, budgetW: 10, checks: 9, scalar: 7, gain: 0.7335, tools: ['pro-axiom-transform', 'axiom-override', 'axiom-sandbox', 'first-principles-lift', 'proof-chain', 'logic-proof'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 11, fanout: 6, temp: 0.28, budgetW: 16, checks: 13, scalar: 11, gain: 1, tools: ['pro-axiom-transform', 'axiom-override', 'axiom-sandbox', 'first-principles-lift', 'proof-chain', 'logic-proof'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return AxiomEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = AxiomEngine.TIER[t], prev = AxiomEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = AxiomEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = AxiomEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return AxiomEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new AxiomEngine();
export default singleton;
export const ENGINE_KEY = "axiom";
