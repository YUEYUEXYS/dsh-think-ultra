// ============================================================================
// replay-engine.js — 独立滑杆引擎：回溯重放（Failure Replay）
...
export class ReplayEngine {
  constructor() {
    this.key = "replay";
    this.name = "回溯重放";
    this.nameEn = "Failure Replay";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "失败链重放深度（己方+对手双重视角）";
    this.toolPool = Object.freeze(['failure-replay', 'root-cause-heal', 'falsify-redteam']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['failure-replay', 'root-cause-heal'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.66, budgetW: 1, checks: 2, scalar: 1, gain: 0.1545, tools: ['failure-replay', 'root-cause-heal'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 2, temp: 0.58, budgetW: 2, checks: 3, scalar: 2, gain: 0.3078, tools: ['failure-replay', 'root-cause-heal', 'falsify-redteam'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 2, temp: 0.46, budgetW: 3, checks: 4, scalar: 2, gain: 0.5019, tools: ['failure-replay', 'root-cause-heal', 'falsify-redteam'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 3, temp: 0.34, budgetW: 5, checks: 6, scalar: 3, gain: 0.7335, tools: ['failure-replay', 'root-cause-heal', 'falsify-redteam'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 5, fanout: 4, temp: 0.26, budgetW: 8, checks: 9, scalar: 5, gain: 1, tools: ['failure-replay', 'root-cause-heal', 'falsify-redteam'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ReplayEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ReplayEngine.TIER[t], prev = ReplayEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ReplayEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ReplayEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ReplayEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ReplayEngine();
export default singleton;
export const ENGINE_KEY = "replay";
