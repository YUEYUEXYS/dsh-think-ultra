// ============================================================================
// vocr-engine.js — 独立滑杆引擎：密集文本誊抄（Dense OCR Transcription）
...
export class VocrEngine {
  constructor() {
    this.key = "vocr";
    this.name = "密集文本誊抄";
    this.nameEn = "Dense OCR Transcription";
    this.domain = "vision";
    this.curve = "fact";
    this.scalarNote = "重叠条带扫读的密度与表格/坐标轴转写粒度";
    this.toolPool = Object.freeze(['dense-ocr', 'table-transcribe', 'ocr-transcribe', 'subpixel-read', 'vision-tile', 'grid-sweep']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.70, budgetW: 1, checks: 1, scalar: 3, gain: 0.0159, tools: ['dense-ocr', 'table-transcribe'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 4, temp: 0.62, budgetW: 2, checks: 2, scalar: 6, gain: 0.0476, tools: ['dense-ocr', 'table-transcribe'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 6, temp: 0.54, budgetW: 3, checks: 3, scalar: 9, gain: 0.1111, tools: ['dense-ocr', 'table-transcribe', 'ocr-transcribe'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 9, temp: 0.42, budgetW: 5, checks: 5, scalar: 13, gain: 0.2381, tools: ['dense-ocr', 'table-transcribe', 'ocr-transcribe', 'subpixel-read'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 12, temp: 0.30, budgetW: 8, checks: 7, scalar: 18, gain: 0.4921, tools: ['dense-ocr', 'table-transcribe', 'ocr-transcribe', 'subpixel-read', 'vision-tile'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 18, temp: 0.20, budgetW: 13, checks: 11, scalar: 26, gain: 1, tools: ['dense-ocr', 'table-transcribe', 'ocr-transcribe', 'subpixel-read', 'vision-tile', 'grid-sweep'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VocrEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VocrEngine.TIER[t], prev = VocrEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VocrEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VocrEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VocrEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VocrEngine();
export default singleton;
export const ENGINE_KEY = "vocr";
