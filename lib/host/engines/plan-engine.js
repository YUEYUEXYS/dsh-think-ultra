// ============================================================================
// plan-engine.js — 独立滑杆引擎：任务拆解规划（Task Decomposition）
...
export class PlanEngine {
  constructor() {
    this.key = "plan";
    this.name = "任务拆解规划";
    this.nameEn = "Task Decomposition";
    this.domain = "engineering";
    this.curve = "exp";
    this.scalarNote = "拆解到可独立验收叶子任务的数量";
    this.toolPool = Object.freeze(['task-decompose', 'work-breakdown', 'dep-graph', 'agent-topology', 'symbolic-deps', 'exec-verify']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 2, gain: 0.0475, tools: ['task-decompose', 'work-breakdown'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 2, temp: 0.64, budgetW: 2, checks: 2, scalar: 4, gain: 0.1545, tools: ['task-decompose', 'work-breakdown'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 3, temp: 0.56, budgetW: 3, checks: 3, scalar: 7, gain: 0.3078, tools: ['task-decompose', 'work-breakdown', 'dep-graph'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 4, temp: 0.44, budgetW: 5, checks: 5, scalar: 11, gain: 0.5019, tools: ['task-decompose', 'work-breakdown', 'dep-graph', 'agent-topology'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 6, temp: 0.32, budgetW: 8, checks: 7, scalar: 16, gain: 0.7335, tools: ['task-decompose', 'work-breakdown', 'dep-graph', 'agent-topology', 'symbolic-deps'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 9, temp: 0.24, budgetW: 13, checks: 10, scalar: 24, gain: 1, tools: ['task-decompose', 'work-breakdown', 'dep-graph', 'agent-topology', 'symbolic-deps', 'exec-verify'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return PlanEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = PlanEngine.TIER[t], prev = PlanEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = PlanEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = PlanEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return PlanEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new PlanEngine();
export default singleton;
export const ENGINE_KEY = "plan";
