// ============================================================================
// cohere-engine.js — 独立滑杆引擎：全链收敛（Global Coherence）
...
export class CohereEngine {
  constructor() {
    this.key = "cohere";
    this.name = "全链收敛";
    this.nameEn = "Global Coherence";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "全局一致性扫描遍数";
    this.toolPool = Object.freeze(['coherence-scan', 'formal-cohere', 'confidence-calib']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['coherence-scan', 'formal-cohere'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.62, budgetW: 1, checks: 2, scalar: 1, gain: 0.1545, tools: ['coherence-scan', 'formal-cohere'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 1, temp: 0.54, budgetW: 2, checks: 3, scalar: 2, gain: 0.3078, tools: ['coherence-scan', 'formal-cohere', 'confidence-calib'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 2, temp: 0.42, budgetW: 3, checks: 5, scalar: 2, gain: 0.5019, tools: ['coherence-scan', 'formal-cohere', 'confidence-calib'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 3, temp: 0.30, budgetW: 5, checks: 7, scalar: 3, gain: 0.7335, tools: ['coherence-scan', 'formal-cohere', 'confidence-calib'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 4, fanout: 4, temp: 0.22, budgetW: 8, checks: 10, scalar: 4, gain: 1, tools: ['coherence-scan', 'formal-cohere', 'confidence-calib'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return CohereEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = CohereEngine.TIER[t], prev = CohereEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = CohereEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = CohereEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return CohereEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new CohereEngine();
export default singleton;
export const ENGINE_KEY = "cohere";
