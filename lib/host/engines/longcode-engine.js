// ============================================================================
// longcode-engine.js — 独立滑杆引擎：长程耐久（Long-Haul Focus）
...
export class LongcodeEngine {
  constructor() {
    this.key = "longcode";
    this.name = "长程耐久";
    this.nameEn = "Long-Haul Focus";
    this.domain = "stability";
    this.curve = "inv";
    this.scalarNote = "对账周期：每 N 个子步骤对账原始目标（反向）";
    this.toolPool = Object.freeze(['longcode-anchor', 'memory-anchor', 'coherence-scan']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 8, gain: 0.2253, tools: ['longcode-anchor', 'memory-anchor'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.66, budgetW: 1, checks: 2, scalar: 6, gain: 0.4331, tools: ['longcode-anchor', 'memory-anchor'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 1, temp: 0.60, budgetW: 2, checks: 3, scalar: 5, gain: 0.6211, tools: ['longcode-anchor', 'memory-anchor', 'coherence-scan'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.50, budgetW: 3, checks: 4, scalar: 4, gain: 0.7852, tools: ['longcode-anchor', 'memory-anchor', 'coherence-scan'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 2, temp: 0.40, budgetW: 5, checks: 6, scalar: 3, gain: 0.9186, tools: ['longcode-anchor', 'memory-anchor', 'coherence-scan'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 3, temp: 0.30, budgetW: 7, checks: 8, scalar: 2, gain: 1, tools: ['longcode-anchor', 'memory-anchor', 'coherence-scan'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return LongcodeEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = LongcodeEngine.TIER[t], prev = LongcodeEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = LongcodeEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = LongcodeEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return LongcodeEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new LongcodeEngine();
export default singleton;
export const ENGINE_KEY = "longcode";
