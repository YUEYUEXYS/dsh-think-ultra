// ============================================================================
// verify-engine.js — 独立滑杆引擎：交叉审判（Judge Panel）
...
export class VerifyEngine {
  constructor() {
    this.key = "verify";
    this.name = "交叉审判";
    this.nameEn = "Judge Panel";
    this.domain = "metacog";
    this.curve = "fact";
    this.scalarNote = "独立审查视角数（事实/逻辑/反例三路）";
    this.toolPool = Object.freeze(['judge-panel', 'dual-recheck', 'falsify-redteam', 'best-of']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.30, budgetW: 1, checks: 1, scalar: 1, gain: 0.0159, tools: ['judge-panel', 'dual-recheck'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.28, budgetW: 1, checks: 2, scalar: 1, gain: 0.0476, tools: ['judge-panel', 'dual-recheck'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 2, temp: 0.24, budgetW: 2, checks: 3, scalar: 2, gain: 0.1111, tools: ['judge-panel', 'dual-recheck', 'falsify-redteam'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 2, temp: 0.20, budgetW: 3, checks: 4, scalar: 2, gain: 0.2381, tools: ['judge-panel', 'dual-recheck', 'falsify-redteam'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 3, temp: 0.16, budgetW: 5, checks: 6, scalar: 3, gain: 0.4921, tools: ['judge-panel', 'dual-recheck', 'falsify-redteam', 'best-of'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 3, fanout: 3, temp: 0.12, budgetW: 7, checks: 8, scalar: 3, gain: 1, tools: ['judge-panel', 'dual-recheck', 'falsify-redteam', 'best-of'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VerifyEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VerifyEngine.TIER[t], prev = VerifyEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VerifyEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VerifyEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VerifyEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VerifyEngine();
export default singleton;
export const ENGINE_KEY = "verify";
