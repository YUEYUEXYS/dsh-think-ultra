// ============================================================================
// deps-engine.js — 独立滑杆引擎：依赖校验（Dependency Graph）
// 领域：engineering；非线性族：线性 lin
...
export class DepsEngine {
  constructor() {
    this.key = "deps";
    this.name = "依赖校验";
    this.nameEn = "Dependency Graph";
    this.domain = "engineering";
    this.curve = "lin";
    this.scalarNote = "依赖图形式化校验强度（拓扑/成环/悬空）";
    this.toolPool = Object.freeze(['dep-graph', 'symbolic-deps', 'agent-topology']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 1, gain: 0.1667, tools: ['dep-graph', 'symbolic-deps'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.62, budgetW: 1, checks: 2, scalar: 2, gain: 0.3333, tools: ['dep-graph', 'symbolic-deps'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.54, budgetW: 2, checks: 3, scalar: 3, gain: 0.5, tools: ['dep-graph', 'symbolic-deps', 'agent-topology'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.44, budgetW: 3, checks: 4, scalar: 4, gain: 0.6667, tools: ['dep-graph', 'symbolic-deps', 'agent-topology'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 5, fanout: 3, temp: 0.34, budgetW: 5, checks: 6, scalar: 5, gain: 0.8333, tools: ['dep-graph', 'symbolic-deps', 'agent-topology'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 8, fanout: 4, temp: 0.26, budgetW: 8, checks: 8, scalar: 8, gain: 1, tools: ['dep-graph', 'symbolic-deps', 'agent-topology'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return DepsEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = DepsEngine.TIER[t], prev = DepsEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = DepsEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = DepsEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return DepsEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new DepsEngine();
export default singleton;
export const ENGINE_KEY = "deps";
