// ============================================================================
// recursion-engine.js — 独立滑杆引擎：自指递归深度（Self-Reference Recursion）
...
export class RecursionEngine {
  constructor() {
    this.key = "recursion";
    this.name = "自指递归深度";
    this.nameEn = "Self-Reference Recursion";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "思考→审视思考→审视审视规则 的最大自缠绕层数";
    this.toolPool = Object.freeze(['self-ref-loop', 'recursion-governor', 'pro-recursion-ctrl', 'metacog-monitor', 'first-principles-lift', 'proof-chain']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['self-ref-loop', 'recursion-governor'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.68, budgetW: 2, checks: 2, scalar: 2, gain: 0.1545, tools: ['self-ref-loop', 'recursion-governor'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.60, budgetW: 3, checks: 3, scalar: 3, gain: 0.3078, tools: ['self-ref-loop', 'recursion-governor', 'pro-recursion-ctrl'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.48, budgetW: 6, checks: 5, scalar: 5, gain: 0.5019, tools: ['self-ref-loop', 'recursion-governor', 'pro-recursion-ctrl', 'metacog-monitor'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 3, temp: 0.36, budgetW: 9, checks: 7, scalar: 8, gain: 0.7335, tools: ['self-ref-loop', 'recursion-governor', 'pro-recursion-ctrl', 'metacog-monitor', 'first-principles-lift'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 5, temp: 0.26, budgetW: 14, checks: 10, scalar: 12, gain: 1, tools: ['self-ref-loop', 'recursion-governor', 'pro-recursion-ctrl', 'metacog-monitor', 'first-principles-lift', 'proof-chain'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return RecursionEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = RecursionEngine.TIER[t], prev = RecursionEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = RecursionEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = RecursionEngine.CHECK_POOL;
    const out = [];
  ...
  enhance(tier, ctx) {
    const t = this.clamp(tier);
    if (t <= 0) return '';
    const p = RecursionEngine.TIER[t];
  ...
  coreMatrix() {
    return RecursionEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new RecursionEngine();
export default singleton;
export const ENGINE_KEY = "recursion";
