// ============================================================================
// falsify-engine.js — 独立滑杆引擎：反事实证伪（Falsification）
...
export class FalsifyEngine {
  constructor() {
    this.key = "falsify";
    this.name = "反事实证伪";
    this.nameEn = "Falsification";
    this.domain = "metacog";
    this.curve = "fact";
    this.scalarNote = "红队攻击轮数：对己方结论构造反例的轮次";
    this.toolPool = Object.freeze(['falsify-redteam', 'dual-recheck', 'best-of', 'apex-tournament']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 1, gain: 0.0159, tools: ['falsify-redteam', 'dual-recheck'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 2, temp: 0.66, budgetW: 2, checks: 2, scalar: 2, gain: 0.0476, tools: ['falsify-redteam', 'dual-recheck'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 3, temp: 0.58, budgetW: 4, checks: 4, scalar: 3, gain: 0.1111, tools: ['falsify-redteam', 'dual-recheck', 'best-of'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 5, fanout: 4, temp: 0.46, budgetW: 6, checks: 6, scalar: 4, gain: 0.2381, tools: ['falsify-redteam', 'dual-recheck', 'best-of', 'apex-tournament'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 7, fanout: 6, temp: 0.34, budgetW: 10, checks: 8, scalar: 6, gain: 0.4921, tools: ['falsify-redteam', 'dual-recheck', 'best-of', 'apex-tournament'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 11, fanout: 9, temp: 0.24, budgetW: 15, checks: 12, scalar: 9, gain: 1, tools: ['falsify-redteam', 'dual-recheck', 'best-of', 'apex-tournament'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return FalsifyEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = FalsifyEngine.TIER[t], prev = FalsifyEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = FalsifyEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = FalsifyEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return FalsifyEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new FalsifyEngine();
export default singleton;
export const ENGINE_KEY = "falsify";
