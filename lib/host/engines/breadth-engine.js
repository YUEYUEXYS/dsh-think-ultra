// ============================================================================
// breadth-engine.js — 独立滑杆引擎：候选路径广度（Candidate Breadth）
...
export class BreadthEngine {
  constructor() {
    this.key = "breadth";
    this.name = "候选路径广度";
    this.nameEn = "Candidate Breadth";
    this.domain = "cognition";
    this.curve = "fact";
    this.scalarNote = "剪枝前一次性铺开的候选假设数量";
    this.toolPool = Object.freeze(['candidate-cast', 'flash-branch', 'branch-tree', 'diverge-converge', 'best-of', 'entropy-prune', 'flash-prune']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.82, budgetW: 1, checks: 1, scalar: 2, gain: 0.0159, tools: ['candidate-cast', 'flash-branch'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 3, temp: 0.76, budgetW: 2, checks: 2, scalar: 4, gain: 0.0476, tools: ['candidate-cast', 'flash-branch'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 5, temp: 0.70, budgetW: 4, checks: 3, scalar: 6, gain: 0.1111, tools: ['candidate-cast', 'flash-branch', 'branch-tree'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 5, fanout: 8, temp: 0.60, budgetW: 6, checks: 5, scalar: 10, gain: 0.2381, tools: ['candidate-cast', 'flash-branch', 'branch-tree', 'diverge-converge'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 7, fanout: 12, temp: 0.48, budgetW: 10, checks: 7, scalar: 15, gain: 0.4921, tools: ['candidate-cast', 'flash-branch', 'branch-tree', 'diverge-converge', 'best-of'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 10, fanout: 18, temp: 0.36, budgetW: 16, checks: 10, scalar: 22, gain: 1, tools: ['candidate-cast', 'flash-branch', 'branch-tree', 'diverge-converge', 'best-of', 'entropy-prune'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return BreadthEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = BreadthEngine.TIER[t], prev = BreadthEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = BreadthEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = BreadthEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return BreadthEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new BreadthEngine();
export default singleton;
export const ENGINE_KEY = "breadth";
