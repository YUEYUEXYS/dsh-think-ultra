// ============================================================================
// format-engine.js — 独立滑杆引擎：结构化输出约束（Schema Output Lock）
// 领域：engineering；非线性族：安全收紧 tight
...
export class FormatEngine {
  constructor() {
    this.key = "format";
    this.name = "结构化输出约束";
    this.nameEn = "Schema Output Lock";
    this.domain = "engineering";
    this.curve = "tight";
    this.scalarNote = "交付前对照结构契约的校验遍数";
    this.toolPool = Object.freeze(['schema-output', 'format-lock', 'formal-cohere', 'logic-proof', 'concept-define', 'exec-verify']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 1, gain: 0.1667, tools: ['schema-output', 'format-lock'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.64, budgetW: 1, checks: 2, scalar: 1, gain: 0.3333, tools: ['schema-output', 'format-lock'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 1, temp: 0.58, budgetW: 1, checks: 3, scalar: 2, gain: 0.5, tools: ['schema-output', 'format-lock', 'formal-cohere'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 1, temp: 0.50, budgetW: 2, checks: 4, scalar: 2, gain: 0.6667, tools: ['schema-output', 'format-lock', 'formal-cohere'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 2, temp: 0.42, budgetW: 3, checks: 5, scalar: 3, gain: 0.8333, tools: ['schema-output', 'format-lock', 'formal-cohere', 'logic-proof'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 4, fanout: 2, temp: 0.34, budgetW: 4, checks: 7, scalar: 4, gain: 1, tools: ['schema-output', 'format-lock', 'formal-cohere', 'logic-proof', 'concept-define'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return FormatEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = FormatEngine.TIER[t], prev = FormatEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = FormatEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = FormatEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return FormatEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new FormatEngine();
export default singleton;
export const ENGINE_KEY = "format";
