// ============================================================================
// retrieval-engine.js — 独立滑杆引擎：证据检索强度（Evidence Retrieval）
...
export class RetrievalEngine {
  constructor() {
    this.key = "retrieval";
    this.name = "证据检索强度";
    this.nameEn = "Evidence Retrieval";
    this.domain = "metacog";
    this.curve = "lin";
    this.scalarNote = "每个承重结论必须挂载的证据条数";
    this.toolPool = Object.freeze(['evidence-retrieval', 'source-ground', 'premise-check', 'confidence-calib', 'bayes-calib', 'coherence-scan']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 1, gain: 0.1667, tools: ['evidence-retrieval', 'source-ground'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.66, budgetW: 1, checks: 2, scalar: 2, gain: 0.3333, tools: ['evidence-retrieval', 'source-ground'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.60, budgetW: 2, checks: 3, scalar: 3, gain: 0.5, tools: ['evidence-retrieval', 'source-ground', 'premise-check'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.52, budgetW: 3, checks: 4, scalar: 5, gain: 0.6667, tools: ['evidence-retrieval', 'source-ground', 'premise-check'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 5, fanout: 3, temp: 0.42, budgetW: 5, checks: 6, scalar: 7, gain: 0.8333, tools: ['evidence-retrieval', 'source-ground', 'premise-check', 'confidence-calib'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 8, fanout: 4, temp: 0.32, budgetW: 8, checks: 9, scalar: 10, gain: 1, tools: ['evidence-retrieval', 'source-ground', 'premise-check', 'confidence-calib', 'bayes-calib', 'coherence-scan'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return RetrievalEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = RetrievalEngine.TIER[t], prev = RetrievalEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = RetrievalEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = RetrievalEngine.CHECK_POOL;
    const out = [];
  ...
  enhance(tier, ctx) {
    const t = this.clamp(tier);
    if (t <= 0) return '';
    const p = RetrievalEngine.TIER[t];
  ...
  coreMatrix() {
    return RetrievalEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new RetrievalEngine();
export default singleton;
export const ENGINE_KEY = "retrieval";
