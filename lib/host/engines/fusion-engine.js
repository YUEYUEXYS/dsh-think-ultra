// ============================================================================
// fusion-engine.js — 独立滑杆引擎：多路径贝叶斯融合（Multi-Path Bayesian Fusion）
// 领域：metacog；非线性族：阶乘跃升 fact
...
export class FusionEngine {
  constructor() {
    this.key = "fusion";
    this.name = "多路径贝叶斯融合";
    this.nameEn = "Multi-Path Bayesian Fusion";
    this.domain = "metacog";
    this.curve = "fact";
    this.scalarNote = "在对数几率空间融合的独立证据/路径数量";
    this.toolPool = Object.freeze(['bayes-fuse-multi', 'evidence-posterior', 'bayes-calib', 'confidence-calib', 'global-equation-solver', 'cross-domain-synth']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.72, budgetW: 1, checks: 1, scalar: 2, gain: 0.0159, tools: ['bayes-fuse-multi', 'evidence-posterior'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 3, temp: 0.66, budgetW: 2, checks: 2, scalar: 3, gain: 0.0476, tools: ['bayes-fuse-multi', 'evidence-posterior'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 4, temp: 0.58, budgetW: 4, checks: 3, scalar: 5, gain: 0.1111, tools: ['bayes-fuse-multi', 'evidence-posterior', 'bayes-calib'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 6, temp: 0.46, budgetW: 6, checks: 5, scalar: 7, gain: 0.2381, tools: ['bayes-fuse-multi', 'evidence-posterior', 'bayes-calib', 'confidence-calib'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 9, temp: 0.34, budgetW: 10, checks: 7, scalar: 10, gain: 0.4921, tools: ['bayes-fuse-multi', 'evidence-posterior', 'bayes-calib', 'confidence-calib', 'global-equation-solver'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 13, temp: 0.24, budgetW: 15, checks: 11, scalar: 15, gain: 1, tools: ['bayes-fuse-multi', 'evidence-posterior', 'bayes-calib', 'confidence-calib', 'global-equation-solver', 'cross-domain-synth'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return FusionEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = FusionEngine.TIER[t], prev = FusionEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = FusionEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = FusionEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return FusionEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new FusionEngine();
export default singleton;
export const ENGINE_KEY = "fusion";
