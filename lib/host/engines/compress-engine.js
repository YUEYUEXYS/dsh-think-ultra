// ============================================================================
// compress-engine.js — 独立滑杆引擎：上下文压缩复用（Context Compression）
// 领域：stability；非线性族：反向收敛 inv
// 稳定主轴：档越深，长程耐久、防环、自愈与回收越强，保证超久不崩、不漂。
...
export class CompressEngine {
  constructor() {
    this.key = "compress";
    this.name = "上下文压缩复用";
    this.nameEn = "Context Compression";
    this.domain = "stability";
    this.curve = "inv";
    this.scalarNote = "过程性上下文压缩百分比（正向，越大压得越狠但硬约束零丢失）";
    this.toolPool = Object.freeze(['context-compress', 'resource-reclaim', 'memory-anchor', 'longcode-anchor', 'lossy-freeze', 'entropy-prune']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 12, gain: 0.2253, tools: ['context-compress', 'resource-reclaim'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.70, budgetW: 1, checks: 2, scalar: 22, gain: 0.4331, tools: ['context-compress', 'resource-reclaim'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 1, temp: 0.66, budgetW: 2, checks: 3, scalar: 32, gain: 0.6211, tools: ['context-compress', 'resource-reclaim', 'memory-anchor'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 2, temp: 0.60, budgetW: 3, checks: 4, scalar: 42, gain: 0.7852, tools: ['context-compress', 'resource-reclaim', 'memory-anchor'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 2, temp: 0.52, budgetW: 5, checks: 6, scalar: 52, gain: 0.9186, tools: ['context-compress', 'resource-reclaim', 'memory-anchor', 'longcode-anchor'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 3, temp: 0.44, budgetW: 7, checks: 8, scalar: 62, gain: 1, tools: ['context-compress', 'resource-reclaim', 'memory-anchor', 'longcode-anchor', 'lossy-freeze'] },
  ]);

  // 本引擎全部“交付前自检”候选池（按档位 checks 数取前 N 条）
  static CHECK_POOL = Object.freeze([
    "子任务严格按依赖序推进，上游未验收下游不开始",
    "无进展重复达到阈值立即收敛，不空转不死循环",
    "失败先等价重试，再检查点回退，再换路/拆分",
    "过程性草稿、废弃分支、失效假设即时回收",
    "长程任务的命名、口径、设计决策前后一致",
    "定期对账原始需求，任何偏离立即拉回",
    "局部失败被隔离，整体任务不被单点错误终止",
    "并行分片给明确输入/输出/验收契约，汇总方交叉核对",
    "同类失败在本任务内不重复触发，根因被抽象为规则",
    "任意长度任务首尾自洽，压缩与展开双向无损",
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return CompressEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = CompressEngine.TIER[t], prev = CompressEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = CompressEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = CompressEngine.CHECK_POOL;
    const out = [];
  ...
  enhance(tier, ctx) {
    const t = this.clamp(tier);
    if (t <= 0) return '';
    const p = CompressEngine.TIER[t];
    const d = this.delta(t);
    const c = ctx || {};
    const head = '【Think 非线性内核·' + this.name + ' 档' + t + '/6·' + p.labelZh + '】';
    const lines = [head];
    lines.push('执行强度 ' + Math.round(p.gain * 100) + '%（' + "反向收敛 inv" + '）；较上一档：迭代 +' + d.iter + '、并行扇出 +' + d.fanout + '、预算权重 +' + d.budgetW + '、自检项 +' + d.checks + '。');
  ...
  coreMatrix() {
    return CompressEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new CompressEngine();
export default singleton;
export const ENGINE_KEY = "compress";
