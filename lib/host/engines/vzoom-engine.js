// ============================================================================
// vzoom-engine.js — 独立滑杆引擎：局部放大巡检（ROI Zoom Sweep）
...
export class VzoomEngine {
  constructor() {
    this.key = "vzoom";
    this.name = "局部放大巡检";
    this.nameEn = "ROI Zoom Sweep";
    this.domain = "vision";
    this.curve = "exp";
    this.scalarNote = "按信息密度排序后逐块放大精读的 ROI 数量";
    this.toolPool = Object.freeze(['zoom-inspect', 'roi-magnify', 'vision-tile', 'subpixel-read', 'grid-sweep', 'ocr-transcribe']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.72, budgetW: 1, checks: 1, scalar: 3, gain: 0.0475, tools: ['zoom-inspect', 'roi-magnify'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 4, temp: 0.64, budgetW: 2, checks: 2, scalar: 6, gain: 0.1545, tools: ['zoom-inspect', 'roi-magnify'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 6, temp: 0.56, budgetW: 4, checks: 4, scalar: 9, gain: 0.3078, tools: ['zoom-inspect', 'roi-magnify', 'vision-tile'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 9, temp: 0.44, budgetW: 6, checks: 6, scalar: 13, gain: 0.5019, tools: ['zoom-inspect', 'roi-magnify', 'vision-tile', 'subpixel-read'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 13, temp: 0.32, budgetW: 9, checks: 8, scalar: 18, gain: 0.7335, tools: ['zoom-inspect', 'roi-magnify', 'vision-tile', 'subpixel-read', 'grid-sweep'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 19, temp: 0.22, budgetW: 14, checks: 12, scalar: 26, gain: 1, tools: ['zoom-inspect', 'roi-magnify', 'vision-tile', 'subpixel-read', 'grid-sweep', 'ocr-transcribe'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VzoomEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VzoomEngine.TIER[t], prev = VzoomEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VzoomEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VzoomEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VzoomEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VzoomEngine();
export default singleton;
export const ENGINE_KEY = "vzoom";
