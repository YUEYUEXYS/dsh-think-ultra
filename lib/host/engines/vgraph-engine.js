// ============================================================================
// vgraph-engine.js — 独立滑杆引擎：空间构境（Spatial Build）
...
export class VgraphEngine {
  constructor() {
    this.key = "vgraph";
    this.name = "空间构境";
    this.nameEn = "Spatial Build";
    this.domain = "vision";
    this.curve = "exp";
    this.scalarNote = "关系拓扑重建深度（遮挡/透视/数据流）";
    this.toolPool = Object.freeze(['spatial-graph', 'image-crosscheck', 'grid-sweep']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.74, budgetW: 1, checks: 1, scalar: 2, gain: 0.0475, tools: ['spatial-graph', 'image-crosscheck'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 3, temp: 0.66, budgetW: 1, checks: 2, scalar: 3, gain: 0.1545, tools: ['spatial-graph', 'image-crosscheck'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 4, temp: 0.58, budgetW: 2, checks: 3, scalar: 4, gain: 0.3078, tools: ['spatial-graph', 'image-crosscheck', 'grid-sweep'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 5, temp: 0.46, budgetW: 4, checks: 5, scalar: 5, gain: 0.5019, tools: ['spatial-graph', 'image-crosscheck', 'grid-sweep'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 6, temp: 0.34, budgetW: 6, checks: 7, scalar: 6, gain: 0.7335, tools: ['spatial-graph', 'image-crosscheck', 'grid-sweep'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 4, fanout: 8, temp: 0.24, budgetW: 10, checks: 10, scalar: 8, gain: 1, tools: ['spatial-graph', 'image-crosscheck', 'grid-sweep'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VgraphEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VgraphEngine.TIER[t], prev = VgraphEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VgraphEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VgraphEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VgraphEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VgraphEngine();
export default singleton;
export const ENGINE_KEY = "vgraph";
