// ============================================================================
// critique-engine.js — 独立滑杆引擎：轻量自驳（Lightweight Self-Critique）
...
export class CritiqueEngine {
  constructor() {
    this.key = "critique";
    this.name = "轻量自驳";
    this.nameEn = "Lightweight Self-Critique";
    this.domain = "metacog";
    this.curve = "fact";
    this.scalarNote = "每轮主动构造的最强反驳数量";
    this.toolPool = Object.freeze(['self-critique-lite', 'falsify-redteam', 'judge-panel', 'dual-recheck', 'fallacy-scan', 'premise-check']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 1, gain: 0.0159, tools: ['self-critique-lite', 'falsify-redteam'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.68, budgetW: 1, checks: 2, scalar: 1, gain: 0.0476, tools: ['self-critique-lite', 'falsify-redteam'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 2, temp: 0.60, budgetW: 2, checks: 3, scalar: 2, gain: 0.1111, tools: ['self-critique-lite', 'falsify-redteam', 'judge-panel'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 2, temp: 0.48, budgetW: 4, checks: 5, scalar: 3, gain: 0.2381, tools: ['self-critique-lite', 'falsify-redteam', 'judge-panel', 'dual-recheck'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 3, temp: 0.36, budgetW: 6, checks: 7, scalar: 4, gain: 0.4921, tools: ['self-critique-lite', 'falsify-redteam', 'judge-panel', 'dual-recheck', 'fallacy-scan'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 4, temp: 0.26, budgetW: 10, checks: 10, scalar: 6, gain: 1, tools: ['self-critique-lite', 'falsify-redteam', 'judge-panel', 'dual-recheck', 'fallacy-scan', 'premise-check'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return CritiqueEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = CritiqueEngine.TIER[t], prev = CritiqueEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = CritiqueEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = CritiqueEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return CritiqueEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new CritiqueEngine();
export default singleton;
export const ENGINE_KEY = "critique";
