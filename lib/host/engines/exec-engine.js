// ============================================================================
// exec-engine.js — 独立滑杆引擎：演算验证（Exec-Verify）
...
export class ExecEngine {
  constructor() {
    this.key = "exec";
    this.name = "演算验证";
    this.nameEn = "Exec-Verify";
    this.domain = "engineering";
    this.curve = "lin";
    this.scalarNote = "独立复算次数：用第二种方法重算关键结论的遍数";
    this.toolPool = Object.freeze(['exec-verify', 'dual-recheck', 'dep-graph']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 1, gain: 0.1667, tools: ['exec-verify', 'dual-recheck'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.60, budgetW: 1, checks: 2, scalar: 2, gain: 0.3333, tools: ['exec-verify', 'dual-recheck'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.50, budgetW: 2, checks: 3, scalar: 3, gain: 0.5, tools: ['exec-verify', 'dual-recheck', 'dep-graph'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.40, budgetW: 4, checks: 5, scalar: 4, gain: 0.6667, tools: ['exec-verify', 'dual-recheck', 'dep-graph'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 3, temp: 0.30, budgetW: 6, checks: 7, scalar: 5, gain: 0.8333, tools: ['exec-verify', 'dual-recheck', 'dep-graph'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 5, temp: 0.22, budgetW: 10, checks: 10, scalar: 8, gain: 1, tools: ['exec-verify', 'dual-recheck', 'dep-graph'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ExecEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ExecEngine.TIER[t], prev = ExecEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ExecEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ExecEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ExecEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ExecEngine();
export default singleton;
export const ENGINE_KEY = "exec";
