// ============================================================================
// topology-engine.js — 独立滑杆引擎：推理拓扑复杂度（Reasoning Topology）
...
export class TopologyEngine {
  constructor() {
    this.key = "topology";
    this.name = "推理拓扑复杂度";
    this.nameEn = "Reasoning Topology";
    this.domain = "engineering";
    this.curve = "fact";
    this.scalarNote = "推理图节点数（线性→对立→陪审团→树/网/环/分形/自缠绕）";
    this.toolPool = Object.freeze(['topology-compose', 'graph-scheduler', 'agent-topology', 'pro-depgraph', 'symbolic-deps', 'cognitive-tournament']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 2, gain: 0.0159, tools: ['topology-compose', 'graph-scheduler'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 2, temp: 0.68, budgetW: 2, checks: 2, scalar: 4, gain: 0.0476, tools: ['topology-compose', 'graph-scheduler'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 4, temp: 0.60, budgetW: 4, checks: 3, scalar: 7, gain: 0.1111, tools: ['topology-compose', 'graph-scheduler', 'agent-topology'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 5, fanout: 6, temp: 0.48, budgetW: 7, checks: 5, scalar: 11, gain: 0.2381, tools: ['topology-compose', 'graph-scheduler', 'agent-topology', 'pro-depgraph'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 7, fanout: 9, temp: 0.36, budgetW: 11, checks: 8, scalar: 16, gain: 0.4921, tools: ['topology-compose', 'graph-scheduler', 'agent-topology', 'pro-depgraph', 'symbolic-deps'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 10, fanout: 14, temp: 0.26, budgetW: 17, checks: 12, scalar: 24, gain: 1, tools: ['topology-compose', 'graph-scheduler', 'agent-topology', 'pro-depgraph', 'symbolic-deps', 'cognitive-tournament'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return TopologyEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = TopologyEngine.TIER[t], prev = TopologyEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = TopologyEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = TopologyEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return TopologyEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new TopologyEngine();
export default singleton;
export const ENGINE_KEY = "topology";
