// ============================================================================
// code-engine.js — 独立滑杆引擎：代码工程强度（Code Engineering）
...
export class CodeEngine {
  constructor() {
    this.key = "code";
    this.name = "代码工程强度";
    this.nameEn = "Code Engineering";
    this.domain = "engineering";
    this.curve = "exp";
    this.scalarNote = "调用链走查轮数：每轮对改动做多轮自审走查";
    this.toolPool = Object.freeze(['exec-verify', 'dep-graph', 'symbolic-deps', 'root-cause-heal', 'formal-cohere']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 2, scalar: 1, gain: 0.0475, tools: ['exec-verify', 'dep-graph'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.60, budgetW: 2, checks: 3, scalar: 2, gain: 0.1545, tools: ['exec-verify', 'dep-graph', 'symbolic-deps'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.50, budgetW: 3, checks: 4, scalar: 3, gain: 0.3078, tools: ['exec-verify', 'dep-graph', 'symbolic-deps'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 5, fanout: 3, temp: 0.38, budgetW: 6, checks: 6, scalar: 4, gain: 0.5019, tools: ['exec-verify', 'dep-graph', 'symbolic-deps', 'root-cause-heal'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 8, fanout: 4, temp: 0.26, budgetW: 9, checks: 9, scalar: 5, gain: 0.7335, tools: ['exec-verify', 'dep-graph', 'symbolic-deps', 'root-cause-heal', 'formal-cohere'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 12, fanout: 6, temp: 0.18, budgetW: 14, checks: 13, scalar: 8, gain: 1, tools: ['exec-verify', 'dep-graph', 'symbolic-deps', 'root-cause-heal', 'formal-cohere'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return CodeEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = CodeEngine.TIER[t], prev = CodeEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = CodeEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = CodeEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return CodeEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new CodeEngine();
export default singleton;
export const ENGINE_KEY = "code";
