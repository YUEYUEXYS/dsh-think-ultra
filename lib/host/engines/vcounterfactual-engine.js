// ============================================================================
// vcounterfactual-engine.js — 独立滑杆引擎：视觉反事实推演（Visual Counterfactual）
// 领域：vision；非线性族：指数增益 exp
// 视觉主轴：档越深，分区/网格/多尺度扫描越细，每个读数都要像素证据。
...
export class VcounterfactualEngine {
  constructor() {
    this.key = "vcounterfactual";
    this.name = "视觉反事实推演";
    this.nameEn = "Visual Counterfactual";
    this.domain = "vision";
    this.curve = "exp";
    this.scalarNote = "对承重视觉元素做移除/改动假设的反事实场景数";
    this.toolPool = Object.freeze(['visual-whatif', 'occlusion-reason', 'vision-counterfactual', 'vision-blindspot', 'vision-critique', 'image-crosscheck']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['visual-whatif', 'occlusion-reason'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 2, temp: 0.68, budgetW: 2, checks: 2, scalar: 2, gain: 0.1545, tools: ['visual-whatif', 'occlusion-reason'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 2, temp: 0.60, budgetW: 3, checks: 3, scalar: 3, gain: 0.3078, tools: ['visual-whatif', 'occlusion-reason', 'vision-counterfactual'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 3, temp: 0.48, budgetW: 5, checks: 4, scalar: 4, gain: 0.5019, tools: ['visual-whatif', 'occlusion-reason', 'vision-counterfactual'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 4, temp: 0.36, budgetW: 8, checks: 6, scalar: 6, gain: 0.7335, tools: ['visual-whatif', 'occlusion-reason', 'vision-counterfactual', 'vision-blindspot'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 5, fanout: 6, temp: 0.26, budgetW: 12, checks: 9, scalar: 9, gain: 1, tools: ['visual-whatif', 'occlusion-reason', 'vision-counterfactual', 'vision-blindspot', 'vision-critique', 'image-crosscheck'] },
  ]);

  // 本引擎全部“交付前自检”候选池（按档位 checks 数取前 N 条）
  static CHECK_POOL = Object.freeze([
    "每个读数都回指到具体坐标/区域，不凭印象概括",
    "图中文字、数字、标签、按钮逐字誊抄",
    "看不清、被遮挡、分辨率不足处显式标“存疑”",
    "上下/左右/前后遮挡与空间关系交代清楚",
    "远中近多尺度交叉核对，读数冲突全部列出再裁决",
    "图表的坐标轴、单位、图例、拐点、趋势完整转写",
    "相邻区域重叠复扫，防止小字与密集表格漏检",
    "视觉结论必须能被图中像素证据支撑",
    "反色/边缘通道辅助捕获角标、水印、图标",
    "输出“位置→内容→置信度”清单，闭环到用户问题",
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VcounterfactualEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VcounterfactualEngine.TIER[t], prev = VcounterfactualEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VcounterfactualEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VcounterfactualEngine.CHECK_POOL;
    const out = [];
  ...
  enhance(tier, ctx) {
    const t = this.clamp(tier);
    if (t <= 0) return '';
    const p = VcounterfactualEngine.TIER[t];
    const d = this.delta(t);
    const c = ctx || {};
    const head = '【Think 非线性内核·' + this.name + ' 档' + t + '/6·' + p.labelZh + '】';
    const lines = [head];
    lines.push('执行强度 ' + Math.round(p.gain * 100) + '%（' + "指数增益 exp" + '）；较上一档：迭代 +' + d.iter + '、并行扇出 +' + d.fanout + '、预算权重 +' + d.budgetW + '、自检项 +' + d.checks + '。');
  ...
  coreMatrix() {
    return VcounterfactualEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VcounterfactualEngine();
export default singleton;
export const ENGINE_KEY = "vcounterfactual";
