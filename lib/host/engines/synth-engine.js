// ============================================================================
// synth-engine.js — 独立滑杆引擎：跨域综合（Cross-Domain Synth）
...
export class SynthEngine {
  constructor() {
    this.key = "synth";
    this.name = "跨域综合";
    this.nameEn = "Cross-Domain Synth";
    this.domain = "metacog";
    this.curve = "fact";
    this.scalarNote = "调用远域数量：结构同构迁移的源域数";
    this.toolPool = Object.freeze(['cross-domain-synth', 'abstraction-lift', 'best-of']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.84, budgetW: 1, checks: 1, scalar: 1, gain: 0.0159, tools: ['cross-domain-synth', 'abstraction-lift'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 2, temp: 0.80, budgetW: 2, checks: 2, scalar: 2, gain: 0.0476, tools: ['cross-domain-synth', 'abstraction-lift'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 2, temp: 0.74, budgetW: 3, checks: 3, scalar: 2, gain: 0.1111, tools: ['cross-domain-synth', 'abstraction-lift', 'best-of'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 3, temp: 0.64, budgetW: 5, checks: 4, scalar: 3, gain: 0.2381, tools: ['cross-domain-synth', 'abstraction-lift', 'best-of'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 4, temp: 0.52, budgetW: 8, checks: 6, scalar: 4, gain: 0.4921, tools: ['cross-domain-synth', 'abstraction-lift', 'best-of'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 6, temp: 0.42, budgetW: 12, checks: 9, scalar: 6, gain: 1, tools: ['cross-domain-synth', 'abstraction-lift', 'best-of'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return SynthEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = SynthEngine.TIER[t], prev = SynthEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = SynthEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = SynthEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return SynthEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new SynthEngine();
export default singleton;
export const ENGINE_KEY = "synth";
