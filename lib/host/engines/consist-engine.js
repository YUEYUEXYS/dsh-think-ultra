// ============================================================================
// consist-engine.js — 独立滑杆引擎：收敛防环（Anti-Loop）
// 领域：stability；非线性族：安全收紧 tight
...
export class ConsistEngine {
  constructor() {
    this.key = "consist";
    this.name = "收敛防环";
    this.nameEn = "Anti-Loop";
    this.domain = "stability";
    this.curve = "tight";
    this.scalarNote = "强制收敛前允许的无进展重复次数（反向，越小越果断）";
    this.toolPool = Object.freeze(['entropy-prune', 'coherence-scan', 'root-cause-heal']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.70, budgetW: 1, checks: 1, scalar: 3, gain: 0.1667, tools: ['entropy-prune', 'coherence-scan'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.62, budgetW: 1, checks: 2, scalar: 3, gain: 0.3333, tools: ['entropy-prune', 'coherence-scan'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 1, temp: 0.54, budgetW: 2, checks: 3, scalar: 2, gain: 0.5, tools: ['entropy-prune', 'coherence-scan', 'root-cause-heal'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 2, temp: 0.44, budgetW: 2, checks: 4, scalar: 2, gain: 0.6667, tools: ['entropy-prune', 'coherence-scan', 'root-cause-heal'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 2, temp: 0.34, budgetW: 4, checks: 6, scalar: 1, gain: 0.8333, tools: ['entropy-prune', 'coherence-scan', 'root-cause-heal'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 4, fanout: 3, temp: 0.24, budgetW: 6, checks: 8, scalar: 1, gain: 1, tools: ['entropy-prune', 'coherence-scan', 'root-cause-heal'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ConsistEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ConsistEngine.TIER[t], prev = ConsistEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ConsistEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ConsistEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ConsistEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ConsistEngine();
export default singleton;
export const ENGINE_KEY = "consist";
