// ============================================================================
// vscan-engine.js — 独立滑杆引擎：逐格巡图（Grid Sweep）
...
export class VscanEngine {
  constructor() {
    this.key = "vscan";
    this.name = "逐格巡图";
    this.nameEn = "Grid Sweep";
    this.domain = "vision";
    this.curve = "fact";
    this.scalarNote = "网格粒度：三区→九宫格→重叠放大→多尺度三遍";
    this.toolPool = Object.freeze(['grid-sweep', 'vision-tile', 'ocr-transcribe', 'subpixel-read']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 3, temp: 0.70, budgetW: 1, checks: 1, scalar: 3, gain: 0.0159, tools: ['grid-sweep', 'vision-tile'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 9, temp: 0.62, budgetW: 2, checks: 2, scalar: 9, gain: 0.0476, tools: ['grid-sweep', 'vision-tile'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 9, temp: 0.54, budgetW: 3, checks: 3, scalar: 9, gain: 0.1111, tools: ['grid-sweep', 'vision-tile', 'ocr-transcribe'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 12, temp: 0.42, budgetW: 5, checks: 5, scalar: 12, gain: 0.2381, tools: ['grid-sweep', 'vision-tile', 'ocr-transcribe', 'subpixel-read'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 15, temp: 0.30, budgetW: 8, checks: 7, scalar: 15, gain: 0.4921, tools: ['grid-sweep', 'vision-tile', 'ocr-transcribe', 'subpixel-read'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 4, fanout: 21, temp: 0.20, budgetW: 12, checks: 10, scalar: 21, gain: 1, tools: ['grid-sweep', 'vision-tile', 'ocr-transcribe', 'subpixel-read'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VscanEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VscanEngine.TIER[t], prev = VscanEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VscanEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VscanEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VscanEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VscanEngine();
export default singleton;
export const ENGINE_KEY = "vscan";
