// ============================================================================
// vspatial-engine.js — 独立滑杆引擎：空间结构重建（Spatial Rebuild）
...
export class VspatialEngine {
  constructor() {
    this.key = "vspatial";
    this.name = "空间结构重建";
    this.nameEn = "Spatial Rebuild";
    this.domain = "vision";
    this.curve = "exp";
    this.scalarNote = "分层/遮挡/透视关系重建的空间图层数";
    this.toolPool = Object.freeze(['spatial-rebuild', 'depth-layering', 'spatial-graph', 'vision-semantic', 'image-crosscheck', 'subpixel-read']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 2, gain: 0.0475, tools: ['spatial-rebuild', 'depth-layering'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 2, temp: 0.66, budgetW: 2, checks: 2, scalar: 3, gain: 0.1545, tools: ['spatial-rebuild', 'depth-layering'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 3, temp: 0.58, budgetW: 3, checks: 3, scalar: 4, gain: 0.3078, tools: ['spatial-rebuild', 'depth-layering', 'spatial-graph'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 4, temp: 0.46, budgetW: 5, checks: 5, scalar: 6, gain: 0.5019, tools: ['spatial-rebuild', 'depth-layering', 'spatial-graph', 'vision-semantic'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 6, temp: 0.34, budgetW: 8, checks: 7, scalar: 8, gain: 0.7335, tools: ['spatial-rebuild', 'depth-layering', 'spatial-graph', 'vision-semantic', 'image-crosscheck'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 9, temp: 0.24, budgetW: 12, checks: 10, scalar: 12, gain: 1, tools: ['spatial-rebuild', 'depth-layering', 'spatial-graph', 'vision-semantic', 'image-crosscheck', 'subpixel-read'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VspatialEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VspatialEngine.TIER[t], prev = VspatialEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VspatialEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VspatialEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VspatialEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VspatialEngine();
export default singleton;
export const ENGINE_KEY = "vspatial";
