// ============================================================================
// reflect-engine.js — 独立滑杆引擎：二阶反思栈（Second-Order Reflection）
// 领域：metacog；非线性族：指数增益 exp
...
export class ReflectEngine {
  constructor() {
    this.key = "reflect";
    this.name = "二阶反思栈";
    this.nameEn = "Second-Order Reflection";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "反思帧压栈层数（对反思本身再反思）";
    this.toolPool = Object.freeze(['second-order-reflect', 'reflection-stack', 'metacog-monitor', 'adversarial-redteam-ultra', 'first-principles-lift', 'self-distilling']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.74, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['second-order-reflect', 'reflection-stack'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.66, budgetW: 2, checks: 2, scalar: 2, gain: 0.1545, tools: ['second-order-reflect', 'reflection-stack'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.58, budgetW: 3, checks: 3, scalar: 3, gain: 0.3078, tools: ['second-order-reflect', 'reflection-stack', 'metacog-monitor'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 3, temp: 0.46, budgetW: 5, checks: 5, scalar: 4, gain: 0.5019, tools: ['second-order-reflect', 'reflection-stack', 'metacog-monitor', 'adversarial-redteam-ultra'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 4, temp: 0.34, budgetW: 8, checks: 7, scalar: 6, gain: 0.7335, tools: ['second-order-reflect', 'reflection-stack', 'metacog-monitor', 'adversarial-redteam-ultra', 'first-principles-lift'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 6, temp: 0.24, budgetW: 13, checks: 11, scalar: 9, gain: 1, tools: ['second-order-reflect', 'reflection-stack', 'metacog-monitor', 'adversarial-redteam-ultra', 'first-principles-lift', 'self-distilling'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ReflectEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ReflectEngine.TIER[t], prev = ReflectEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ReflectEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ReflectEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ReflectEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ReflectEngine();
export default singleton;
export const ENGINE_KEY = "reflect";
