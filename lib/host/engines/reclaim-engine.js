// ============================================================================
// reclaim-engine.js — 独立滑杆引擎：资源回收（Resource Reclaim）
// 领域：stability；非线性族：线性 lin
...
export class ReclaimEngine {
  constructor() {
    this.key = "reclaim";
    this.name = "资源回收";
    this.nameEn = "Resource Reclaim";
    this.domain = "stability";
    this.curve = "lin";
    this.scalarNote = "过程性上下文压缩百分比（正向，越大回收越狠）";
    this.toolPool = Object.freeze(['resource-reclaim', 'entropy-prune', 'memory-anchor']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 6, gain: 0.1667, tools: ['resource-reclaim', 'entropy-prune'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.66, budgetW: 1, checks: 2, scalar: 12, gain: 0.3333, tools: ['resource-reclaim', 'entropy-prune'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 1, temp: 0.60, budgetW: 2, checks: 2, scalar: 18, gain: 0.5, tools: ['resource-reclaim', 'entropy-prune'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.50, budgetW: 2, checks: 3, scalar: 26, gain: 0.6667, tools: ['resource-reclaim', 'entropy-prune', 'memory-anchor'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 5, fanout: 2, temp: 0.40, budgetW: 3, checks: 4, scalar: 34, gain: 0.8333, tools: ['resource-reclaim', 'entropy-prune', 'memory-anchor'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 3, temp: 0.30, budgetW: 5, checks: 6, scalar: 42, gain: 1, tools: ['resource-reclaim', 'entropy-prune', 'memory-anchor'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ReclaimEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ReclaimEngine.TIER[t], prev = ReclaimEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ReclaimEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ReclaimEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ReclaimEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ReclaimEngine();
export default singleton;
export const ENGINE_KEY = "reclaim";
