// ============================================================================
// crossmodel-engine.js — 独立滑杆引擎：跨模型协同调度（Cross-Model Orchestration）
// 领域：engineering；非线性族：阶乘跃升 fact
...
export class CrossmodelEngine {
  constructor() {
    this.key = "crossmodel";
    this.name = "跨模型协同调度";
    this.nameEn = "Cross-Model Orchestration";
    this.domain = "engineering";
    this.curve = "fact";
    this.scalarNote = "异构模型角色映射数量（主思考/批判/实验/收敛各司其职）";
    this.toolPool = Object.freeze(['crossmodel-rolemap', 'hetero-schedule', 'agent-topology', 'cognitive-tournament', 'pro-depgraph', 'global-equation-solver']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.74, budgetW: 1, checks: 1, scalar: 2, gain: 0.0159, tools: ['crossmodel-rolemap', 'hetero-schedule'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 3, temp: 0.68, budgetW: 2, checks: 2, scalar: 3, gain: 0.0476, tools: ['crossmodel-rolemap', 'hetero-schedule'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 4, temp: 0.60, budgetW: 4, checks: 3, scalar: 5, gain: 0.1111, tools: ['crossmodel-rolemap', 'hetero-schedule', 'agent-topology'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 6, temp: 0.48, budgetW: 6, checks: 5, scalar: 7, gain: 0.2381, tools: ['crossmodel-rolemap', 'hetero-schedule', 'agent-topology', 'cognitive-tournament'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 9, temp: 0.36, budgetW: 10, checks: 7, scalar: 10, gain: 0.4921, tools: ['crossmodel-rolemap', 'hetero-schedule', 'agent-topology', 'cognitive-tournament', 'pro-depgraph'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 13, temp: 0.26, budgetW: 15, checks: 10, scalar: 14, gain: 1, tools: ['crossmodel-rolemap', 'hetero-schedule', 'agent-topology', 'cognitive-tournament', 'pro-depgraph', 'global-equation-solver'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return CrossmodelEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = CrossmodelEngine.TIER[t], prev = CrossmodelEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = CrossmodelEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = CrossmodelEngine.CHECK_POOL;
    const out = [];
  ...
  enhance(tier, ctx) {
    const t = this.clamp(tier);
    if (t <= 0) return '';
    const p = CrossmodelEngine.TIER[t];
  ...
  coreMatrix() {
    return CrossmodelEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new CrossmodelEngine();
export default singleton;
export const ENGINE_KEY = "crossmodel";
