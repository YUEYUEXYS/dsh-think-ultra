// ============================================================================
// vtrack-engine.js — 独立滑杆引擎：多帧时序追踪（Multi-Frame Tracking）
// 领域：vision；非线性族：阶乘跃升 fact
...
export class VtrackEngine {
  constructor() {
    this.key = "vtrack";
    this.name = "多帧时序追踪";
    this.nameEn = "Multi-Frame Tracking";
    this.domain = "vision";
    this.curve = "fact";
    this.scalarNote = "跨帧/多图稳定追踪并对齐的实体数量";
    this.toolPool = Object.freeze(['frame-track', 'multi-frame-align', 'image-crosscheck', 'spatial-graph', 'vision-semantic', 'grid-sweep']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 2, temp: 0.72, budgetW: 1, checks: 1, scalar: 2, gain: 0.0159, tools: ['frame-track', 'multi-frame-align'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 3, temp: 0.64, budgetW: 2, checks: 2, scalar: 3, gain: 0.0476, tools: ['frame-track', 'multi-frame-align'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 4, temp: 0.56, budgetW: 3, checks: 3, scalar: 5, gain: 0.1111, tools: ['frame-track', 'multi-frame-align', 'image-crosscheck'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 3, fanout: 6, temp: 0.44, budgetW: 5, checks: 5, scalar: 7, gain: 0.2381, tools: ['frame-track', 'multi-frame-align', 'image-crosscheck', 'spatial-graph'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 4, fanout: 8, temp: 0.32, budgetW: 8, checks: 7, scalar: 10, gain: 0.4921, tools: ['frame-track', 'multi-frame-align', 'image-crosscheck', 'spatial-graph', 'vision-semantic'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 6, fanout: 12, temp: 0.22, budgetW: 12, checks: 10, scalar: 15, gain: 1, tools: ['frame-track', 'multi-frame-align', 'image-crosscheck', 'spatial-graph', 'vision-semantic', 'grid-sweep'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return VtrackEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = VtrackEngine.TIER[t], prev = VtrackEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = VtrackEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = VtrackEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return VtrackEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new VtrackEngine();
export default singleton;
export const ENGINE_KEY = "vtrack";
