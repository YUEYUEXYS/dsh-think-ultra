// ============================================================================
// abst-engine.js — 独立滑杆引擎：抽象升维（Abstraction Lift）
...
export class AbstEngine {
  constructor() {
    this.key = "abst";
    this.name = "抽象升维";
    this.nameEn = "Abstraction Lift";
    this.domain = "metacog";
    this.curve = "exp";
    this.scalarNote = "递归向上抽象层数：解完通用结构再无损回落";
    this.toolPool = Object.freeze(['abstraction-lift', 'cross-domain-synth', 'formal-cohere']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.82, budgetW: 1, checks: 1, scalar: 1, gain: 0.0475, tools: ['abstraction-lift', 'cross-domain-synth'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.78, budgetW: 2, checks: 2, scalar: 1, gain: 0.1545, tools: ['abstraction-lift', 'cross-domain-synth'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 2, temp: 0.72, budgetW: 3, checks: 3, scalar: 2, gain: 0.3078, tools: ['abstraction-lift', 'cross-domain-synth', 'formal-cohere'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 3, temp: 0.62, budgetW: 5, checks: 4, scalar: 2, gain: 0.5019, tools: ['abstraction-lift', 'cross-domain-synth', 'formal-cohere'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 4, temp: 0.50, budgetW: 8, checks: 6, scalar: 3, gain: 0.7335, tools: ['abstraction-lift', 'cross-domain-synth', 'formal-cohere'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 5, fanout: 6, temp: 0.40, budgetW: 12, checks: 9, scalar: 5, gain: 1, tools: ['abstraction-lift', 'cross-domain-synth', 'formal-cohere'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return AbstEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = AbstEngine.TIER[t], prev = AbstEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = AbstEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = AbstEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return AbstEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new AbstEngine();
export default singleton;
export const ENGINE_KEY = "abst";
