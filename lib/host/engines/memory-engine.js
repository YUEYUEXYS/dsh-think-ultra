// ============================================================================
// memory-engine.js — 独立滑杆引擎：记忆巩固（Memory Anchor）
// 领域：cognition；非线性族：反向收敛 inv
...
export class MemoryEngine {
  constructor() {
    this.key = "memory";
    this.name = "记忆巩固";
    this.nameEn = "Memory Anchor";
    this.domain = "cognition";
    this.curve = "inv";
    this.scalarNote = "锚定周期：每隔 N 轮重申原始目标（反向，越小越强）";
    this.toolPool = Object.freeze(['memory-anchor', 'context-inject', 'longcode-anchor', 'resource-reclaim']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.75, budgetW: 1, checks: 1, scalar: 6, gain: 0.2253, tools: ['memory-anchor', 'context-inject'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 2, fanout: 1, temp: 0.70, budgetW: 1, checks: 2, scalar: 5, gain: 0.4331, tools: ['memory-anchor', 'context-inject'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 3, fanout: 2, temp: 0.65, budgetW: 2, checks: 3, scalar: 4, gain: 0.6211, tools: ['memory-anchor', 'context-inject', 'longcode-anchor'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 4, fanout: 2, temp: 0.55, budgetW: 3, checks: 4, scalar: 3, gain: 0.7852, tools: ['memory-anchor', 'context-inject', 'longcode-anchor'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 6, fanout: 3, temp: 0.45, budgetW: 5, checks: 6, scalar: 2, gain: 0.9186, tools: ['memory-anchor', 'context-inject', 'longcode-anchor', 'resource-reclaim'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 9, fanout: 4, temp: 0.35, budgetW: 8, checks: 9, scalar: 1, gain: 1, tools: ['memory-anchor', 'context-inject', 'longcode-anchor', 'resource-reclaim'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return MemoryEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = MemoryEngine.TIER[t], prev = MemoryEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = MemoryEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = MemoryEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return MemoryEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new MemoryEngine();
export default singleton;
export const ENGINE_KEY = "memory";
