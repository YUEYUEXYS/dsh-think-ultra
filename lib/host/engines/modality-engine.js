// ============================================================================
// modality-engine.js — 独立滑杆引擎：输出模态粒度（Output Modality Granularity）
...
export class ModalityEngine {
  constructor() {
    this.key = "modality";
    this.name = "输出模态粒度";
    this.nameEn = "Output Modality Granularity";
    this.domain = "metacog";
    this.curve = "lin";
    this.scalarNote = "推理过程可见度等级（仅结论→核心步骤→辩论→自省→废枝→原始流）";
    this.toolPool = Object.freeze(['modality-project', 'output-granularity', 'dev-profile', 'self-distilling', 'metacog-monitor', 'confidence-calib']);
  }

  static TIER = Object.freeze([
    { tier: 0, label: 'off', labelZh: '关', iter: 0, fanout: 0, temp: 0.00, budgetW: 0, checks: 0, scalar: 0, gain: 0, tools: [] },
    { tier: 1, label: 'low', labelZh: '低', iter: 1, fanout: 1, temp: 0.72, budgetW: 1, checks: 1, scalar: 1, gain: 0.1667, tools: ['modality-project', 'output-granularity'] },
    { tier: 2, label: 'mid', labelZh: '中', iter: 1, fanout: 1, temp: 0.68, budgetW: 1, checks: 2, scalar: 2, gain: 0.3333, tools: ['modality-project', 'output-granularity'] },
    { tier: 3, label: 'high', labelZh: '高', iter: 2, fanout: 1, temp: 0.64, budgetW: 2, checks: 3, scalar: 3, gain: 0.5, tools: ['modality-project', 'output-granularity', 'dev-profile'] },
    { tier: 4, label: 'ultra', labelZh: '超高', iter: 2, fanout: 2, temp: 0.58, budgetW: 3, checks: 4, scalar: 4, gain: 0.6667, tools: ['modality-project', 'output-granularity', 'dev-profile'] },
    { tier: 5, label: 'limit', labelZh: '极限', iter: 3, fanout: 2, temp: 0.50, budgetW: 4, checks: 6, scalar: 5, gain: 0.8333, tools: ['modality-project', 'output-granularity', 'dev-profile', 'self-distilling'] },
    { tier: 6, label: 'zenith', labelZh: '绝顶', iter: 5, fanout: 3, temp: 0.42, budgetW: 6, checks: 8, scalar: 5, gain: 1, tools: ['modality-project', 'output-granularity', 'dev-profile', 'self-distilling', 'metacog-monitor'] },
  ]);
  ...
  /** 取某一档的不可变参数（0 档为关闭态） */
  at(tier) {
    return ModalityEngine.TIER[this.clamp(tier)];
  }
  ...
  delta(tier) {
    ...
    const cur = ModalityEngine.TIER[t], prev = ModalityEngine.TIER[t - 1];
    return {
  ...
  checklist(tier) {
    const t = this.clamp(tier);
    const n = ModalityEngine.TIER[t].checks;
    if (n <= 0) return [];
    const pool = ModalityEngine.CHECK_POOL;
    const out = [];
  ...
  coreMatrix() {
    return ModalityEngine.TIER.map((p) => ({ t: p.tier, i: p.iter, f: p.fanout, temp: p.temp, b: p.budgetW, c: p.checks, x: p.scalar, gain: p.gain, tools: p.tools.slice() }));
  }
}

const singleton = new ModalityEngine();
export default singleton;
export const ENGINE_KEY = "modality";
